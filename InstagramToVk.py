...
class InstagramToVk(object):
    ...
    def __init__(self, **kwargs):
        default_attr = dict(username='', usernames=[], filename=None,
                            ...
                            verbose=0, include_location=False, filter=None, time_start=int(time.time()) - 300, vk_appi=False,vk_api_arguments=None,
                            dict_text_label=None)
        ...
        self.last_scraped_filemtime = 0#self.time_start
    ...
    def _retry_prompt(self, url, exception_message):
        answer = choice("ir") #input( 'Repeated error {0}\n(A)bort, (I)gnore, (R)etry or retry (F)orever?'.format(exception_message) )
        self.logger.info('Answer response  is {0}:'.format(answer))
        time.sleep(20)
        if answer:
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 404:
                    return
                self.logger.info("request - {0} - {1}".format(args,response))
    ...
    def login(self):
        self.logger.info("Logs in to instagram")
    ...
    def logout(self):
        ...
        self.logger.info("Logs out of instagram.")
    ...
    def __scrape_query(self, media_generator, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS)):
        ...
        try:
            for value in self.usernames:
                self.posts = []
                self.last_scraped_filemtime = self.time_start
    ...
    def augment_node(self, node):
        ...
        try:
            if node['taken_at_timestamp'] >= self.time_start:
                    ...
                    if details:
                        if '__typename' in details and details['__typename'] == 'GraphVideo':
                            ...
                            for carousel_item in details['edge_sidecar_to_children']['edges']:
                                noded = carousel_item['node']
                                noded["taken_at_timestamp"] = node["taken_at_timestamp"]
                                urls += self.augment_node(noded)['urls']
                            node['urls'] = urls
                        else:
    ...
    def scrape(self, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS)):
        ...
        try:
            for username in self.usernames:
                ...
                #data_post_photo = self.deep_get(user,'edges')
...
#        self.is_newData()
    ...
    def query_media_gen(self, user, end_cursor=''):
        ...
        media, end_cursor = self.__query_media(user['id'],user, end_cursor)
    ...
    def __query_media(self, id, user,end_cursor=''):
    ...
    def textToVK_post(self,username):
        try:
            textpost= self.dict_text_label[username]
        except:
            textpost = None
        return textpost

    def loadImagetovkCommunity(self,file_path,save_dir):

        try:
            text_post = self.textToVK_post(username=save_dir.split('/')[1])

            with open(file_path, 'rb') as media_files:
                vk = vk_api.VkApi(login=self.vk_api_arguments['login'], password=self.vk_api_arguments['password'], token=self.vk_api_arguments['Vk_token'])
                vk.auth()
                getUploadServer = vk.method('photos.getUploadServer', {'group_id': self.vk_api_arguments['Vk_group_id'],'album_id': self.vk_api_arguments['Vk_album_id']})
                file_json = requests.post(getUploadServer['upload_url'], files={'photo': media_files}).json()
                saveWallPhoto = vk.method('photos.save', {'group_id': self.vk_api_arguments['Vk_group_id'],
                                                          'album_id': self.vk_api_arguments['Vk_album_id'],
                                                          'caption': text_post,
                                                          'photos_list': file_json['photos_list'],
                                                          'server': file_json['server'], 'hash': file_json['hash']})[0]
                self.logger.info("post : {0}".format(saveWallPhoto))
        except :
            self.logger.exception('Not posted photo from intagram  account - {0} after  - {1}'.format(save_dir.split('/')[1], self.time_start))
    ...
    def download(self, item, save_dir='./'):
        ...
        for url in item['urls']:
            ...
            if self.vk_appi is True:
                self.loadImagetovkCommunity(file_path=file_path,save_dir=save_dir)
    ...
    def is_new_media(self, item):
        ...
        return current_timestamp > 0 and current_timestamp >= self.time_start#self.last_scraped_filemtime
    ...
    def get_logger(level=logging.DEBUG, verbose=0):
        ...
        if logger.handlers ==[]:
            fh = logging.FileHandler('instagram-scraper.log', 'a')
